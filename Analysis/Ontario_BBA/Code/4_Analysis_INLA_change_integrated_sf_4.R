...
# dirname <- thisPath()
od_user_path <- Sys.getenv("OneDrive")
dirname <- file.path(od_user_path, "NOBMWG_data")
setwd(dirname)

covar_tbl_pth <- "derived-data/INLA_results/Tables_Summaries/covariate_effect_table.csv"
rel_abund_map_pth <- "derived-data/INLA_results/maps/Relative_Abundance/"
chg_tbl_pth <- "derived-data/INLA_results/Tables_Summaries/OBBA3_change_estimate_table.csv"
`%!in%` <- Negate(`%in%`)
...
# set folder for eBird ranges
Sys.setenv(EBIRDST_DATA_DIR = file.path(dirname, "raw-data/eBird_ranges"))

analysis_data <- readRDS("derived-data/INLA_data/analysis_data_package.rds")
...
ON_BCR <- st_read("raw-data/BCR_Terrestrial/BCR_Terrestrial_master.shp")  %>%
...
ONBoundary <- st_read("raw-data/BCR_Terrestrial/BCR_Terrestrial_master.shp")  %>%
  subset(PROVINCE_S == "ONTARIO") %>%
  st_make_valid() %>%
  st_transform(AEA_proj) %>%
  st_buffer(5) %>%
...
if (file.exists(paste0(chg_tbl_pth))) change_estimate_table <- read.csv(paste0(chg_tbl_pth), header = TRUE)

# Empty table to populate with summaries of covariate effects
covariate_effect_table <- data.frame()
if (file.exists(paste0(covar_tbl_pth))) covariate_effect_table <- read.csv(paste0(covar_tbl_pth), header = TRUE)

species_to_model <- left_join(species_to_model %>% select(-Removal, -Distance),
...
species_list <- c("Dark-eyed Junco", "Blackburnian Warbler")
for (spec in species_list){
  ...
  for (etype in c("nbinomial")){
    ...
    if (file.exists(paste0(rel_abund_map_pth,species_name,"_",etype,".png"))) next
    ...
    write.csv(covariate_effect_table,paste0(covar_tbl_pth), row.names = FALSE)
    ...
    png(file = paste0(rel_abund_map_pth,species_name,"_",etype,".png"),
        units = "in",
        width = 40,
        height = 14, res = 600)
    ...
    write.csv(change_estimate_table,paste0(chg_tbl_pth), row.names = FALSE)
